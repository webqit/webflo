...
export async function generate() {
    ...
    // -----------
    // Scan Subdocuments
    function scanSubroots(sparoot, rootFileName) {
    ...
    async function generateClient(sparoot, spaGraphCallback = null) {
    ...
    async function generateWorker(workerroot, workerGraphCallback = null) {
    ...
    // -----------
    // Generate now...
    const sparootsFile = Path.join(dirPublic, 'sparoots.json');
...
function declareRoutesObj(gen, routesDir, targetDir, varName, routing) {
    ...
    if (routing.name === 'client') {
        // ------------------
        // >> MHR
        gen.code.push(`// >> HMR Hook`);
        gen.code.push(`${varName}.replace = async (route, moduleUrl) => {`);
        gen.code.push(`    ${varName}[route] = await import(\`@handler/\${moduleUrl}?t=\${Date.now()}\`);`);
        gen.code.push(`};`);
    }
    if (!handlerCount) {
...
async function bundle(gen, outfile, asModule = false) {
    ...
    // Main
    const buildResult = await EsBuild.build(bundlingConfig);
    // Compress...
